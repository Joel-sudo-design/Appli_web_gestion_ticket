...
function generateTicketAccordion(obj, formatImg, type) {
    ...
    <div id="accordion-body-${obj.id}" class="accordion-body align-item-center">
          ...
          <div id="row-accordion-answer-${obj.id}" class="row justify-content-start">
...
function generateAnswerRow(obj, index) {
    return `
    <div id="row-answer-${obj.id}-${index}" class="row mt-3 justify-content-start">
...
function generateSendButton(obj, answerIndex) {
    return `<div id="row-send-answer-${obj.id}-${answerIndex}" class="text-center">
                <button id="sendAnswerButton-${obj.id}-${answerIndex}" class="submit fs-6 mt-2 load-hidden">ENVOYER</button>
            </div>`;
}
function generateAnswersRows(obj) {
    ...
    return obj.answers.map((answer, index) => {
        ...
        const justifyClass = role === "user" ? "justify-content-start" : "justify-content-end";
        const admin = role ==="admin" ? "<div style='font-size:0.8rem'>Réponse de l'administateur</div>" : '';
            ...
            <div class="accordion accordion-flush">
              <div class="accordion-item border">
                <h2 class="accordion-header">
                  <button class="pt-0 pb-0 flex-column accordion-button RotateArrow collapsed align-items-start" 
                  id="button-ticket-${obj.id}" type="button">
...
function attachTicketEvents(obj, type) {
    ...
    if (type === "inProgress") {
        ...
        $buttonAnswer.on("click", function () {
            ...
            // Génération du bouton "ENVOYER" après le textarea
            if ($rowAnswer.length !== 0) {
                const sendBoutonHtml = generateSendButton(obj, answerIndex);
                $rowAnswer.after(sendBoutonHtml);
            }

            // Animation avec ScrollReveal du bouton "ENVOYER"
            const $buttonSendAnswer = $(`#sendAnswerButton-${obj.id}-${answerIndex}`);
            const rowSendAnswer = $(`#row-send-answer-${obj.id}-${answerIndex}`);
            ScrollReveal().reveal(rowSendAnswer[0], {
                origin: 'bottom',
                distance: '100px',
                duration: 400,
                delay: 200,
                easing: "ease-in-out",
                beforeReveal: function () { $buttonSendAnswer.removeClass('load-hidden'); },
            });
